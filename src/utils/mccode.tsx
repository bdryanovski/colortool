...
// Types
export type RGBAcss = string;
export type HEXcolor = string;

function checkRange(value: number, maxValue: number, label: string): void {
  if (isNaN(value) || 0 > value || value > maxValue)
    throw new RangeError(value + ' for ' + label + ' is not between 0 and ' + maxValue);
}

var ACCURACY: number = Math.pow(2, -16);
...
export function rgb2hex(rgbColor: RGBColor) {
  return (
...
var normalizeRGB = function (rgbColor: RGBColor): RGBColor {
  return 1 - rgbColor.alpha < ACCURACY ? rgbColor : new RGBColor(rgbColor.red, rgbColor.green, rgbColor.blue);
};

var contrastRatio = function (rgbColor1: RGBColor, rgbColor2: RGBColor): number {
  ...
  const rgbColor1ratio: number = 0.2126 * rgb2xyz(rgbColor1.red) + 0.7152 * rgb2xyz(rgbColor1.green) + 0.0722 * rgb2xyz(rgbColor1.blue);
  const rgbColor2ratio: number = 0.2126 * rgb2xyz(rgbColor2.red) + 0.7152 * rgb2xyz(rgbColor2.green) + 0.0722 * rgb2xyz(rgbColor2.blue);
  return rgbColor1ratio >= rgbColor2ratio ? (rgbColor1ratio + 0.05) / (rgbColor2ratio + 0.05) : (rgbColor2ratio + 0.05) / (rgbColor1ratio + 0.05);
};

var kb = function (a: number, b: number, c: number, d: number): RGBColor {
  var e = d,
    g = d;
  a = (a % 360) / 60;
...
function decimal2hex(decimal: number): string {
...
class HSLColor {
  ...
  public rotate(hueAdjustment: number): HSLColor {
